...
def build_pairs(hazy_dir: str, gt_dir: str, expected_per_group: int | None = None):
    ...
    if expected_per_group is not None:
        ...
        if bad:
            ...
            print(f"⚠️ 번호별 hazy 개수 기대치({expected_per_group})와 다름 (일부): {sample}")
        else:
...
# ─── Train 페어 ───
hazy_files, ref_files = build_pairs(hazy_dir, ref_dir, expected_per_group=35)  # train은 hazy 35장 가정
contexts = [''] * len(hazy_files)  # 길이 맞춤

print("✔ 최종 train_hazy 페어 길이:", len(hazy_files))
print("✔ 최종 train_ref 페어 길이:", len(ref_files))

# ─── Test 페어 ───
# test도 hazy가 다중이면 expected_per_group=35, 1:1이면 1 또는 None로 두세요.
hazy_test_files, ref_test_files = build_pairs(hazy_test_dir, ref_test_dir, expected_per_group=35)
contexts_test = [''] * len(hazy_test_files)

print("✔ 최종 test_hazy  페어 길이:", len(hazy_test_files))
print("✔ 최종 test_ref  페어 길이:", len(ref_test_files))
...
context_embedding_dir = '/home/jang/DDIM_python/paper/contexts_blip_embedding_RESIDE-b'

context_embedding_dir_test = '/home/jang/DDIM_python/paper/contexts_blip_embedding_test_RESIDE-b'
...
val_ds = (
    ...
    .shuffle(4096, reshuffle_each_iteration=False) # 한 번만 섞어서 고정
    .repeat()
    .batch(batch_siz)
    .prefetch(tf.data.AUTOTUNE)
)
...
steps_per_epoch = int(tf.math.ceil(len(hazy_files) / effective_batch_siz)) # 6000 / batch_siz
...
class ResidualBlock(layers.Layer):  # 입력 : (B, sz, sz, C)
    def __init__(self, w, **kwargs):
        ...
        G = min(groups, w)           # 채널보다 큰 그룹 방지
        if w % G != 0:               # 나눠떨어지도록 보정(예: 가장 가까운 약수로)
            for g in range(G, 0, -1):
                if w % g == 0:
                    G = g; break
        self.norm = layers.GroupNormalization(groups=G, axis=-1, epsilon=1e-5)
