...
class DiffusionModel(tf.keras.Model):
    ...
    def plot_images(self, epoch=None, logs=None, val_ds=None, ihaze_ds=None, num_rows=3, num_cols=6): #이 변수로 개수 조절
...
# checkpoint_path = "/home/jang/DDIM_python/paper/checkpoints_weights/epoch010-valNoise0.42839.net.weights.h5"   # 가중치 불러오기
# # 모델에 가중치 로드
...
inference_model = DiffusionModel(img_siz, widths, block_depth, ctx_dim)

# 2. 저장된 가장 좋은 가중치를 불러옵니다.
best_weights_path = os.path.join(checkpoint_dir, "best.weights.h5")
inference_model.load_weights(best_weights_path)
print(f"가중치 로드 완료: {best_weights_path}")

# 3. 모델을 컴파일합니다. (추론만 할 때는 간단하게 해도 됩니다)
inference_model.compile(optimizer=optimizers.Adam())

# 4. 검증 데이터셋에서 한 배치를 가져옵니다.
val_iterator = iter(val_ds)
hazy_b, clear_b, ctx_b = next(val_iterator)

# 5. plot_images 함수를 직접 호출하여 이미지 생성 및 저장
#    이제 이 함수는 깨끗한 GPU 메모리 위에서 실행됩니다.
print("이미지 생성 중...")
inference_model.plot_images(epoch="final", val_ds=val_ds.take(1)) # val_ds.take(1)로 한 배치만 전달
print("✅ 최종 이미지가 'samples/generated_epoch_final.png' 에 저장되었습니다.")
